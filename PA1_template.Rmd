...
Steps by day:
```{r Counting_the_steps_in_each_day}
...
```{r Calculating_the_mean_of_the_steps_by_interval_with_NAs}
...
```{r Calculating_the mean_of_thesteps_by_interval_without_NAs}
data4work<-mutate(data4work,interval=factor(data4work$interval))
...
```{r Plotting_the_Daily_behaviour_Scatterplot}
...
```{r Substituting_missing_values_with_the_behavioural_mean}
...
```{r Counting_steps_by_day_again,echo=FALSE}
...
```{r Statistical_data_with_imputed_values_instead_of_NAs}
...
```{r Creating_the_factor_column_week_or_weekend}
...
```{r summarizing_for_the_weekdays_and_weekends}
